...
//void odomCallback(const sensor_msgs::JointState::ConstPtr& msg,const sensor_msgs::JointState::ConstPtr& msg2)
...
//        <<" v ave: "<<Vave<<" yaw: "<<m.yaw<<" m.curr_x: "<<m.curr_x<<" m.curr_y: "<<m.curr_y<<std::endl;
    ...
    if(encoder_path.poses.size()%100==0) m.encoder_path_pub.publish(encoder_path);
}

void lla2enu(const Eigen::Vector3d &init_lla,
...
//    msg->header.stamp=ros::Time(gps_msg->header.stamp.sec+gps_msg->header.stamp.nsec*1e-6);
    ...
    if(!gps_init_flag)
    {
        ori_lla<<msg->pose.pose.position.y,msg->pose.pose.position.x,msg->pose.pose.position.z;//ned
    ...
    c_lla<<msg->pose.pose.position.y,msg->pose.pose.position.x,msg->pose.pose.position.z; //ned
...
//    std::cout<<  " c_enu "<< c_enu<<std::endl;
    ...
    double last_pose_dis= sqrt(pow(gps_pose.pose.position.x-c_enu.x(),2)+pow(gps_pose.pose.position.y-c_enu.y(),2));

    if(msg->pose.covariance[0]<m.gps_cov_threshold && (last_pose_dis > m.gps_interval_threshold))
    {
...
bool cali_cbk(std_srvs::Empty::Request& req, std_srvs::Empty::Response& res)
{
    ...
    std::cout<<"gps: "<<seg_size<<" encoder_size: "<<encoder_size <<std::endl;
...
//    Eigen::Vector3d gps2wheel(0, 0.26, 0); //right
    ...
    for(int k=0;k<iter_num;k++)
    {
        ...
        for(int i=0;i<seg_size;i++)
        {
            ...
            {
                double dt=0;
                if(imu_buffer.at(imu_idx)->header.stamp > gps_path.poses[i+1].header.stamp)
                {
                    if(imu_idx>0) last_imu=imu_idx-1;
                    break;
                }
                if(imu_buffer.at(imu_idx+1)->header.stamp < gps_path.poses[i].header.stamp)
                {
                    continue;
                }
                if(imu_buffer.at(imu_idx)->header.stamp <= gps_path.poses[i].header.stamp && imu_buffer.at(imu_idx+1)->header.stamp >= gps_path.poses[i].header.stamp)
                    dt = (imu_buffer.at(imu_idx+1)->header.stamp - gps_path.poses[i].header.stamp).toSec();
                else if(imu_buffer.at(imu_idx)->header.stamp <= gps_path.poses[i+1].header.stamp && imu_buffer.at(imu_idx+1)->header.stamp >= gps_path.poses[i+1].header.stamp)
                    dt = (gps_path.poses[i+1].header.stamp - imu_buffer.at(imu_idx)->header.stamp).toSec();
                else dt = (imu_buffer.at(imu_idx+1)->header.stamp - imu_buffer.at(imu_idx)->header.stamp).toSec();

                imu_yaw +=imu_buffer.at(imu_idx)->angular_velocity.z*dt; //3dm imu roll=M_PI
            }
            gps_yaw +=imu_yaw;
            gps_path.poses.at(i+1).pose.position.z=gps_yaw;
            // VIWO formula 26 RT*RT*(T_K_1*P-T_K*P)
            gps_k_pos<<gps_path.poses[i].pose.position.x,gps_path.poses[i].pose.position.y,0;
            gps_k_1_pos<<gps_path.poses[i+1].pose.position.x,gps_path.poses[i+1].pose.position.y,0;
            tq.x()=temp_q.x;
            tq.y()=temp_q.y;
            tq.z()=temp_q.z;
            tq.w()=temp_q.w;
            ...
            g2v_odom = gps_k_rot.transpose()*((gps_k_1_rot*gps2wheel+gps_k_1_pos)-(gps_k_rot*gps2wheel+gps_k_pos));
            ...
            {
                double dt=0;
                if(encoder_inc_path.poses[j].header.stamp > gps_path.poses[i+1].header.stamp)
                {
                    if(j>0) last_j=j-1;
                    break;
                }
                if(encoder_inc_path.poses[j+1].header.stamp < gps_path.poses[i].header.stamp) continue;

                if(encoder_inc_path.poses[j].header.stamp < gps_path.poses[i].header.stamp && encoder_inc_path.poses[j+1].header.stamp > gps_path.poses[i].header.stamp)
                    dt = (encoder_inc_path.poses[j+1].header.stamp - gps_path.poses[i].header.stamp).toSec();
                else if(encoder_inc_path.poses[j].header.stamp < gps_path.poses[i+1].header.stamp && encoder_inc_path.poses[j+1].header.stamp > gps_path.poses[i+1].header.stamp)
                    dt = (gps_path.poses[i+1].header.stamp - encoder_inc_path.poses[j].header.stamp).toSec();
                else dt = (encoder_inc_path.poses[j+1].header.stamp - encoder_inc_path.poses[j].header.stamp).toSec();

                dt /=(encoder_inc_path.poses[j+1].header.stamp - encoder_inc_path.poses[j].header.stamp).toSec();
                ...
                encoder_inc_xytheta(0) += (vl + vr) * cos(encoder_inc_xytheta(2)) / 2.0f;
                encoder_inc_xytheta(1) += (vl + vr) * sin(encoder_inc_xytheta(2)) / 2.0f;
                encoder_H(0,0) += encoder_inc_path.poses[j].pose.position.x * cos(encoder_inc_xytheta(2)) / 2.0f * dt;
                encoder_H(0,1) += encoder_inc_path.poses[j].pose.position.y * cos(encoder_inc_xytheta(2)) / 2.0f * dt;
                encoder_H(1,0) += encoder_inc_path.poses[j].pose.position.x * sin(encoder_inc_xytheta(2)) / 2.0f * dt;
                encoder_H(1,1) += encoder_inc_path.poses[j].pose.position.y * sin(encoder_inc_xytheta(2)) / 2.0f * dt;
                encoder_H(2,0) -= encoder_inc_path.poses[j].pose.position.x / m.wheel_track * dt;
                encoder_H(2,1) += encoder_inc_path.poses[j].pose.position.y / m.wheel_track * dt;
                encoder_H(2,2) -= (vr - vl)/ m.wheel_track  / m.wheel_track;
//                    encoder_H(0,2) += (vl + vr) * sin(encoder_inc_xytheta(2)) / 2.0f*
//                                      (vr - vl)/ m.wheel_track  / m.wheel_track;
//                    encoder_H(1,2) -= (vl + vr) * cos(encoder_inc_xytheta(2)) / 2.0f*
//                                      (vr - vl)/ m.wheel_track  / m.wheel_track;
            }
            ...
            b(3*i) -= g2v_odom[0] * cos(gps_rot_yaw) - g2v_odom[1] * sin(gps_rot_yaw);
            b(3*i+1) -= g2v_odom[0] * sin(gps_rot_yaw) + g2v_odom[1] * cos(gps_rot_yaw);
            b(3*i+2) -= imu_yaw;
            H(3*i,3) -= -g2v_odom[0] * sin(gps_rot_yaw) - g2v_odom[1] * cos(gps_rot_yaw);
            H(3*i+1,3) -= g2v_odom[0] * cos(gps_rot_yaw) - g2v_odom[1] * sin(gps_rot_yaw);
...
//        std::cout<<"b: " <<b<<std::endl;
        ...
        gps2wheel[0] -=r_x[4];
        gps2wheel[1] -=r_x[5];
        std::cout<< k <<" iter calculate result: "<<m.wheel_radius_l <<" "<<m.wheel_radius_r<<" "<<m.wheel_track <<" "<< gps_rot_yaw << " "<<gps2wheel.transpose() <<std::endl;
    }
...
//visualize refined path
    ...
    refined_encoder_path.header.stamp=encoder_inc_path.poses.back().header.stamp;
    ...
    for(int i=0;i<gps_path.poses.size();i++)
    {
        ...
        gps_xy <<  gps_path.poses[i].pose.position.x,gps_path.poses[i].pose.position.y;
        gps_g0_gk_yaw << cos(gps_path.poses[i].pose.position.z),-sin(gps_path.poses[i].pose.position.z),sin(gps_path.poses[i].pose.position.z),cos(gps_path.poses[i].pose.position.z);
        gps_g_v_yaw <<cos(gps_rot_yaw),-sin(gps_rot_yaw),sin(gps_rot_yaw),cos(gps_rot_yaw);
    ...
    ofs.open("Calibrated.txt", std::ios::out);
    if (!ofs.is_open()) {
...
int main(int argc, char** argv)
{
    ...
    m.refined_encoder_path_pub = nh.advertise<nav_msgs::Path>("refined_encoder_path", 1000);
...
//    typedef message_filters::sync_policies::ApproximateTime<sensor_msgs::JointState, sensor_msgs::JointState> syncpolicy;//时间戳对齐规则
...
//    sync_->registerCallback(boost::bind(&odomCallback, _1, _2));
    ...
    ros::Subscriber joint_sub = nh.subscribe("/mini_hercules/encoder", 200000, odomCallback);

    ros::Subscriber sub = nh.subscribe("/3dm_ins/nav/odom", 200000, gnss_cbk);
//    ros::Subscriber sub = nh.subscribe("/3dm_ins/gnss_right/odom", 1000, gnss_cbk);
    ros::ServiceServer service = nh.advertiseService("cali_encoder", cali_cbk);
    ros::Subscriber sub_imu = nh.subscribe("/3dm_ins/imu/data_raw", 200000, imu_cbk,ros::TransportHints().tcpNoDelay());
