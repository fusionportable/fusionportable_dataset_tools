...
class FileLoader():
  ...
  def load_odometry(self, file_path, traj_type='TUM'):
    """
    Load odometry data from a file.

    Parameters
    ----------
    file_path : str
        Path to the odometry data file.
    traj_type : str, optional
        Type of trajectory data in the file. Can be 'TUM' or 'KITTI'. The default is 'TUM'.

    Returns
    -------
    timestamps : np.ndarray
        Array of odometry timestamps.
    quaternions : np.ndarray
        Array of odometry quaternions. (qx, qy, qz, qw)
    translations : np.ndarray
        Array of odometry translations. (tx, ty, tz)
